...
boolean showingTemp, showingUmid, showingHabitability;

PImage umidImage, tempImage;
...
void showUmid() {
  image(umidImage, 0, 0, width, height);
}

void showHabitability() {
  for (int x = 0; x < width; ++x) {
    for (int y = 0; y < height; ++y) {
      float groupingValue = 16;
      color pix = 255;

      float value = (umidity[x][y]+temperature[x][y])/2.0;
      pix = lerpColor(#FF0004, #02F745, value);
...
void setAmbient() {
  ...
  for (int x = 0; x < width; ++x) {
    for (int y = 0; y < height; ++y) {
      ...
      tempImage.pixels[x+y*width] = color(r, g, b);
      
      pix = 255;


      //pix = lerpColor(#3CBFE5, 255, umidity[x][y]);
      if (umidity[x][y]<.5) {
        pix = lerpColor(#FCF50D, #0A9D57, map(umidity[x][y], 0, 1, 0, 1));
      } else {
        pix = lerpColor(#9CDBFC, #0538FC, map(umidity[x][y], .5, 1, 0, 1));
      }
      ...
      g = round(groupingValue*g/255)*(255/groupingValue);
      b = round(groupingValue*b/255)*(255/groupingValue);
      umidImage.pixels[x+y*width] = color(r, g, b);
    }
  }
  tempImage.updatePixels();
  umidImage.updatePixels();
}
...
void keyReleased() {
  ...
  case 'p':
    ...
    break;
  case 'r':
    if (plantSelected!=null) plantSelected.selected = false;
    plantSelected = plant.get(floor(random(plant.size())));
    plantSelected.selected = true;
    break;
  case '-':
  ...
  if (plantSelected!=null && keyCode == BACKSPACE) {
    plantSelected.selected = false;
    plantSelected = null;
  }
}
...
void simulate() {
  if (time%fps==0) {
    ...
    if (tempIsOpen && umidIsOpen) showHabitability();
    showOrganisms();
    updateOrganisms();
    showplant();
    updateplant();
    ...
    fill(255);
    text(umidity[floor(mouseX)][floor(mouseY)], mouseX, mouseY);
...
void setup() {
  ...
  umidImage = new PImage(width, height);
  setAmbient();
