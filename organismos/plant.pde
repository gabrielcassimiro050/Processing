  ...
  void update() {
    ...
    for (int i = 0; i < plant.size(); ++i) {
      ...
      if (dist(f.pos, pos) < range && dist(f.pos, pos) < f.range) {
        ...
        if (random(1) < .1 && checkDna(f.dna) < plantCompatibility && life >= map(size, 1, 5, 50, 100)*1.5 && f.life >= map(f.size, 1, 5, 50, 100)*1.5 && age>ageExpectancy/2.0 && f.age>f.ageExpectancy/2.0) {
          f.life-=f.size*3;
          reproduce(f);
        }
      }
    ...
    life -= pow(2, (size-temperature[floor(pos.x)][floor(pos.y)])*map(umidity[floor(pos.x)][floor(pos.y)], 0, 1, 0, 2))*age/ageExpectancy;
